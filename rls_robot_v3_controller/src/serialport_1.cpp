/************************************************************************************//**
* \file         port/linux/serialport_1.c
...
#include "../include/rls_robot_v3_controller/serialport_1.h"                     /* serial port module                      */
...
#define SERIALPORT_INVALID_HANDLE_1      (-1)
 ...
 */
static const speed_t baudrateLookup_1[] =
{
...
bool SerialPortOpen_1(char const * portname, tSerialPortBaudrate_1 baudrate) 
{
  ...
  if (portname != NULL) /*lint !e774 */
  {
    ...
    result = true;
    /* Open the port. */
    portHandle_1 = open(portname, O_RDWR | O_NOCTTY | O_NDELAY);
    /* Check the result */
    if (portHandle_1 == SERIALPORT_INVALID_HANDLE_1)
    {
      result = false;
    }
    ...
    if (result)
    {
      if (fcntl(portHandle_1, F_SETFL, 0) == -1)
      {
        SerialPortClose_1();
        result = false;
      }
    ...
    if (result)
    {
      if (tcgetattr(portHandle_1, &options) == -1)
      {
        SerialPortClose_1();
        result = false;
      }
    ...
    if (result)
    {
      if (cfsetispeed(&options, baudrateLookup_1[baudrate]) == -1)
      {
        SerialPortClose_1();
        result = false;
      }
    ...
    if (result)
    {
      if (cfsetospeed(&options, baudrateLookup_1[baudrate]) == -1)
      {
        SerialPortClose_1();
        result = false;
      }
    ...
    if (result)
    {
      ...
      if (tcsetattr(portHandle_1, TCSAFLUSH, &options) == -1)
      {
        SerialPortClose_1();
        result = false;
      }
    ...
    {
      iFlags = TIOCM_DTR;
      if (ioctl(portHandle_1, TIOCMBIS, &iFlags) == -1)
      {
        SerialPortClose_1();
        result = false;
      }
...
void SerialPortClose_1(void)
{
  ...
  /* Invalidate handle. */
  portHandle_1 = SERIALPORT_INVALID_HANDLE_1;
} /*** end of SerialPortClose_1 ***/
...
bool SerialPortWrite_1(uint8_t const * data, uint32_t length)
{
  ...
  if ( (data != NULL) && (length > 0) ) /*lint !e774 */
  {
    ...
    if ((uint32_t)write(portHandle_1, data, length) == length)
    {
      result = true;
    }
  }
